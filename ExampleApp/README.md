...
3. 실기기(Apple Vision Pro) 또는 visionOS 시뮬레이터에서 실행 → "물속으로 들어가기" 버튼

`NSHandsTrackingUsageDescription`은 빌드 설정(`INFOPLIST_KEY_...`)에 이미 포함되어 있습니다.

> 시뮬레이터에서도 심해 돔·수면·물결 애니메이션까지는 보이지만,
> 핸드 트래킹(빛 조정)은 실기기에서만 동작합니다.

## 조작

- **오른손 검지** — 수면 위 태양이 손의 수평 이동을 3배 증폭해 따라옴
- **엄지+검지 핀치** — 쥘수록 빛이 강해지고(최대 40,000lm) 물결이 잔잔해져 빛 기둥이 또렷해짐
