...
final class SphereInteractionManager: ObservableObject {
    private enum InteractionState {
        case idle
        case original(sphere: ModelEntity, position: SIMD3<Float>, clone: ModelEntity)
        case movable(sphere: ModelEntity)
        case overlap(target: ModelEntity, mixedSphere: ModelEntity)
    }
    ...
    func subscribe(to content: RealityViewContent) {
        manipulationSubscription = content.subscribe(to: ManipulationEvents.WillBegin.self) { [weak self] event in
            self?.handleManipulationBegan(event)
        }

        transformSubscription = content.subscribe(to: ManipulationEvents.DidUpdateTransform.self) { [weak self] event in
    ...
    private func selectSphere(_ sphere: ModelEntity) {
        guard selectedSphere !== sphere else { return }
    ...
    private func handleManipulationBegan(_ event: ManipulationEvents.WillBegin) {
        guard let entity = event.entity as? ModelEntity, case .idle = state else { return }
        ...
        if let component = entity.components[OverlapVisualComponent.self] {
            ...
            state = .overlap(target: entity, mixedSphere: mixedSphere)
            selectSphere(mixedSphere)
            return
        }

        // An original sphere creates a movable clone.
        ...
        guard movableSpheres.contains(where: { $0 === entity }) else { return }

        state = .movable(sphere: entity)
        selectSphere(entity)
    }

    private func handleManipulationUpdated(_ event: ManipulationEvents.DidUpdateTransform) {
        guard let entity = event.entity as? ModelEntity else { return }

        switch state {
        case let .original(sphere, position, clone):
            guard entity === sphere else { return }

            // Keep the original sphere fixed while its clone follows the gesture.
        ...
        case let .overlap(target, mixedSphere):
            guard entity === target else { return }
    ...
    private func handleManipulationReleased(_ event: ManipulationEvents.WillRelease) {
        guard let entity = event.entity as? ModelEntity else { return }
    ...
    func deleteAllMovableSpheres() {
        ...
        movableSpheres.forEach { $0.removeFromParent() }
        overlapTargets.forEach { $0.removeFromParent() }
    ...
    private func updateOverlapTargets() {
        ...
        let overlaps = overlapSystem.checkOverlap(spheres: movableSpheres)
        let currentKeys = Set(overlaps.map(\.key))

        // Remove targets whose spheres no longer overlap.
        overlapTargets.removeAll { target in
            guard let component = target.components[OverlapVisualComponent.self] else {
                target.removeFromParent()
                return true
            }
        ...
        for overlap in overlaps {
            if let target = overlapTargets.first(where: {
                ...
                if let parent = target.parent {
                    target.position = parent.convert(position: overlap.position, from: nil)
                }
            ...
            guard let parent = overlap.firstSphere.parent else { continue }
            ...
            target.components.set(
                CollisionComponent(shapes: [.generateSphere(radius: overlapHitRadius)])
            )
    ...
    private func createClone(from original: ModelEntity) -> ModelEntity {
        ...
        let clone = ModelEntity(
            ...
            materials: [makeRGBMaterial(color: color.uiColor)]
        )
    ...
    private func createMixedSphere(overlap: SphereOverlap) -> ModelEntity {
        let mixedSphere = ModelEntity(
            mesh: .generateSphere(radius: sphereRadius),
            materials: [makeRGBMaterial(color: overlap.color.uiColor)]
        )
        ...
        if let parent = overlap.firstSphere.parent {
            mixedSphere.position = parent.convert(position: overlap.position, from: nil)
    ...
    /// Keeps a mixed sphere aligned with its invisible overlap target.
    private func moveMixedSphere(_ sphere: ModelEntity, to target: ModelEntity) {
